...
router.delete("/events/:id", requireAuth, function (request, response) {
  ...
  dynamoDb.delete(params, function (error, data) {
    if (error) {
      console.error(
        "Unable to delete item. Erron JSON:",
        JSON.stringify(error, null, 2)
      );
    } else {
...
router.get("/events/approve", requireAuth, function (request, response) {
  ...
  const params = {
    TableName: EVENTS_TABLE,
    ProjectionExpress:
      "eventID, title, office, start_date, start_time, eventStatus",
      ...
      let eventData = JSON.stringify({
        d: {
          __metadata: {
            ...
            uri:
              "https://spteamfearless.sharepoint.com/sites/SBAEvents/_api/Web/Lists(guid'62accc8a-ae0e-43d7-a1cf-27a8cb78558a')/Items(304)",
            etag: '"2"',
